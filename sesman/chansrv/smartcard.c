...
{
    clog_debug("entered: device_id=%d", device_id);

    if (g_smartcards_inited)
    {
        clog_error("already init");
        return;
    }
    ...
    if (g_scard_index < 0)
        clog_debug("scard_add_new_device failed with DeviceId=%d", g_device_id);
    else
        clog_debug("added smartcard with DeviceId=%d to list", g_device_id);
}
...
scard_add_new_device(tui32 device_id)
{
    ...
    {
        clog_error("scard_get_free_slot failed");
        return -1;
    }
...
scard_get_free_slot(void)
{
    ...
    for (i = 0; i < MAX_SMARTCARDS; i++)
    {
        ...
        {
            clog_debug("found free slot at index %d", i);
            return i;
        }
    }

    clog_error("too many smart card devices; rejecting this one");
    return -1;
}
...
scard_send_EstablishContext(IRP *irp, int scope)
{
    ...
    {
        clog_error("scard_make_new_ioctl failed");
        return;
    }
...
scard_send_ReleaseContext(IRP *irp, char *context, int context_bytes)
{
    ...
    {
        clog_error("smartcards[%d] is NULL", irp->scard_index);
        return;
    }
                  ...
                  int cchReaderLen, int cbAtrLen)
{
    ...
    if ((s = scard_make_new_ioctl(irp, ioctl)) == NULL)
    {
        clog_error("scard_make_new_ioctl");
        return;
    }
/*
              30 00 00 00
                    ...
                    struct xrdp_scard_io_request *recv_ior)
{
    ...
    clog_debug("send_bytes %d recv_bytes %d send dwProtocol %d cbPciLength %d "
                                     ...
                                     tui32 IoStatus)
{
    ...
    {
        clog_error("DeviceId/CompletionId do not match those in IRP");
        return;
    }
    /* get OutputBufferLen */
